...
import { Heart, User, MapPin, AlertCircle, Lock, Eye, EyeOff } from "lucide-react";
...
const donorFormSchema = z.object({
    ...
    age: z.coerce.number().min(18, { message: "বয়স কমপক্ষে ১৮ বছর হতে হবে" }).max(65, { message: "বয়স সর্বোচ্চ ৬৫ বছর হতে পারে" }),
    weight: z.coerce.number().min(45, { message: "ওজন কমপক্ষে ৪৫ কেজি হতে হবে" }),
...
export const DonorForm = ({ onSubmit, defaultValues }: DonorFormProps) => {
    ...
    const availableUpazilas = selectedDistrict ? upazilas[selectedDistrict as keyof typeof upazilas] || [] : [];
...
<CardHeader>
 <CardTitle className="text-2xl text-center">রক্তদাতার তথ্য</CardTitle> <CardDescription className="text-center">
...
<CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2  gap-4">
                    <div className="space-y-2">
                        ...
                        <Input id="name" placeholder="আপনার পূর্ণ নাম" {...register("name")} className={errors.name ? "border-red-500" : ""} />
                        {renderErrorMessage(errors.name?.message)}
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="email">ইমেইল ঠিকানা *</Label>
                        <Input id="email" type="email" placeholder="your@email.com" {...register("email")} className={errors.email ? "border-red-500" : ""} />
                        {renderErrorMessage(errors.email?.message)}
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="phone">মোবাইল নম্বর *</Label>
                        <Input id="phone" placeholder="01XXXXXXXXX" {...register("phone")} className={errors.phone ? "border-red-500" : ""} />
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="age" type="number" placeholder="২৫" {...register("age")} className={errors.age ? "border-red-500" : ""} />
                        {renderErrorMessage(errors.age?.message)}
                    </div>
                    <div className="space-y-2">
                        <Label htmlFor="weight">ওজন (কেজি) *</Label>
                        <Input id="weight" type="number" placeholder="৬০" {...register("weight")} className={errors.weight ? "border-red-500" : ""} />
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="space-y-2">
                        ...
                        <Controller name="division" control={control} render={({ field }) => (
                            <Select value={field.value} onValueChange={(value) => handleDivisionChange(value)}>
                    ...
                    <div className="space-y-2">
                        ...
                        <Controller name="district" control={control} render={({ field }) => (
                            <Select value={field.value} onValueChange={(value) => handleDistrictChange(value)} disabled={!selectedDivision}>
                    ...
                    <div className="space-y-2">
                        <Label>উপজেলা *</Label>
                        <Controller name="upazila" control={control} render={({ field }) => (
                            <Select value={field.value} onValueChange={field.onChange} disabled={!selectedDistrict || availableUpazilas.length === 0}>
                                <SelectTrigger className={errors.upazila ? "border-red-500" : ""}>
            ...
            <div className="space-y-4">
                 <h3 className="text-lg font-semibold text-gray-800">অতিরিক্ত তথ্য</h3>
                 <div className="space-y-2">
                    <Label htmlFor="medicalHistory">চিকিৎসা ইতিহাস (ঐচ্ছিক)</Label>
                    <Textarea id="medicalHistory" placeholder="কোনো গুরুতর অসুখ বা অ্যালার্জি থাকলে উল্লেখ করুন" {...register("medicalHistory")} />
                 </div>
                 <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg">
                    <div>
                        <h4 className="font-medium text-green-800">রক্তদানের জন্য উপলব্ধ</h4>
                        <p className="text-sm text-green-600">জরুরি প্রয়োজনে আপনাকে যোগাযোগ করা হবে</p>
                    </div>
            ...
            {/* Submit Button */}
            <Button type="submit" disabled={isSubmitting} className="cursor-pointer w-full h-12 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-lg font-semibold">
                {isSubmitting ? (
