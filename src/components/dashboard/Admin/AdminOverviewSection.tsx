...
import { useGetAllBloodRequestsQuery } from "@/redux/features/bloodRequest/requestApi";
...
const AdminOverviewSection = () => {
  ...
  // Fetch all requests
  const { data: allRequestsData } = useGetAllBloodRequestsQuery({
    page: 1,
    limit: 1000,
  });

  // Fetch all donors
  const { data: allDonorsData } = useGetAllDonorsQuery({
    page: 1,
    limit: 1000,
  });

  // Fetch recent pending requests
  const { data: recentRequestsData } = useGetAllBloodRequestsQuery({
    page: 1,
    limit: 3,
    status: "Pending",
  });
  ...
  // Calculate stats
  const totalUsers = allDonorsData?.meta?.total || 0;
  const totalDonors = allDonorsData?.meta?.total || 0;
  const activeRequests =
    allRequestsData?.data?.filter((req: any) => req.status === "Pending")
      ?.length || 0;
  const completedRequests =
    allRequestsData?.data?.filter((req: any) => req.status === "Completed")
      ?.length || 0;
  const livesImpacted = completedRequests * 3;

  // System alerts
  const systemAlerts = [
    {
      type: activeRequests > 10 ? "Critical" : "Warning",
      message: `${activeRequests} pending blood requests need attention`,
      time: "Just now",
      resolved: false,
    },
    {
      type: "Info",
      message: `${totalDonors} active donors in the system`,
      time: "5 min ago",
      resolved: false,
    },
    {
      type: "Info",
      message: `${completedRequests} requests completed successfully`,
      time: "1 hour ago",
      resolved: true,
    },
  ];
    ...
    <div className="space-y-6 md:space-y-8 animate-slide-up">
      ...
      <div className="grid gap-6 md:gap-8 lg:grid-cols-2">
        ...
        <Card className="card-hover border-0 shadow-xl bg-gradient-to-br from-white to-red-50/30">
          ...
          <CardContent className="space-y-4">
            {recentRequestsData?.data?.length > 0 ? (
              <>
                {recentRequestsData.data.map((request: any) => (
                  <div
                    key={request._id}
                    className="flex items-center space-x-3 md:space-x-4 p-3 md:p-4 rounded-xl bg-white/60 hover:bg-white/80 transition-colors"
                  >
                    ...
                    <div className="flex flex-col space-y-1">
                      <Button
                        size="sm"
                        onClick={() => handleManageRequest(request._id)}
                        className="bg-green-500 hover:bg-green-600 text-xs px-2"
                      >
                        Manage
                      </Button>
                ...
                >
                  View All Requests ({activeRequests})
                </Button>
              </>
            ) : (
