...
//          { id: "settings", label: "Settings", icon: Settings, color: "text-gray-600", href: "/dashboard/admin/settings" },
//     ]
// };
...
const navItemsData: Record<UserRole, NavItem[]> = {
  ...
  donor: [
    ...
    },
    {
      ...
      icon: Heart,
      color: "text-red-600",
      href: "/dashboard/donor/blood-requests",
    },
    {
      id: "history",
...
export const DashboardSidebar = ({
  ...
  const handleLogout = async () => {
    try {
      ...
      dispatch(logoutAction());
      toast.error(error?.data?.message || "Logout failed, but session cleared");
      navigate("/login-page");
    }
  };
      ...
      <nav className="space-y-2 md:space-y-3 p-6 h-full flex flex-col">
        ...
        <div className="flex-grow space-y-2 md:space-y-3">
          ...
          <button
            ...
            className={cn(
              "w-full justify-start h-12 text-left transition-all duration-200 flex items-center px-4 rounded-lg font-medium",
              "hover:bg-red-50 hover:shadow-md text-red-600 hover:text-red-700",
