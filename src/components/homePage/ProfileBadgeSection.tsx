          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
            {milestones.map((milestone, index) => (
              ...
              >
                <motion.div
                  className={cn(
                    "w-24 h-24 rounded-full flex items-center justify-center border-4 border-white",
                    milestone.shadow
                  )}
                  style={{
                    backgroundColor:
                      milestone.color.replace("text-", "bg-") + "/10",
                  }}
                >
                  <milestone.icon
                    className={cn("w-12 h-12", milestone.color)}
                  />
                </motion.div>
                <h3 className="text-xl font-bold  mt-4">{milestone.title}</h3>
                <p className="text-sm font-semibold text-primary">
                  {milestone.donations} বার রক্তদানের পর
                </p>
              </motion.div>
            ))}
          </div>
